...
const files = [
  ['src/drivers/admin-areas.controller.ts', '/opt/bigbot/server/src/drivers/admin-areas.controller.ts'],
  ['src/drivers/drivers.module.ts',         '/opt/bigbot/server/src/drivers/drivers.module.ts'],
  ['src/drivers/eta.service.ts',            '/opt/bigbot/server/src/drivers/eta.service.ts'],
];
  ...
  conn.sftp((err, sftp) => {
    ...
    function uploadNext() {
      if (i >= files.length) {
        ...
        conn.exec(
          'cd /opt/bigbot/server && npm run build 2>&1 | tail -10 && pm2 restart bigbot-server 2>&1 | tail -5',
          (err2, stream) => {
